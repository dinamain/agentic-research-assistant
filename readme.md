...
START
↓ Planner node — LLM breaks the task into a short numbered plan (advisory, not enforced)
↓
Agent node ←──────────────────────┐
↓ (LLM decides: answer directly,│
↓ call one tool, or call both │
...
↓ │
Tools node │
├─ search_documents (ChromaDB retriever: rewrite → retrieve k=15 →
│ cross-encoder re-rank → relevance threshold → top 5)
└─ tavily_search (web search: Tavily → relevance-score filter → top 3)
↓
back to Agent node ─────────────┘

State (`AgentState`) carries: `messages` (append-only, full conversation/tool history), `plan` (overwrite, set once), `tool_retry_count` (overwrite, resets on success, capped on failure).
